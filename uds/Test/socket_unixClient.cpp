...
int main()
{
  std::string path = "../echo_socket";
  SocketUnix libr;
  int fdClient = libr.newUnixTcpClient(path);
  int fdServer = libr.newUnixTcpServer(path, 2);
  char sendBuff[5] = "Rick";
  libr.sendToUnix(fdClient, path, (void*)sendBuff, 4);
  char recvBuff[100] = "";
  libr.recvFromUnix(fdClient, (void*)recvBuff, 100, (std::string*)"../echo_socket");
